...
class ReservationView(View):
    def get(self, request):
        ...
        reservations = Reservation.objects.select_related("airline","airport_depart","airport_arrive").prefetch_related("flightweekday_set").filter(airport_depart__name = flight_condition['depart'], airport_arrive__name = flight_condition['arrive'], flightweekday__date = flight_condition['depart_date']).filter(q)
        ...
        reservation_list = [{
            ...
            "depart_weekday"     : FlightWeekday.objects.filter(date = flight_condition['depart_date'], reservation_id = reservation.id)[0].weekday,
            ...
            "basic_price"        : reservation.basic_price,
            "seat_class"         : reservation.seat_class
        } for reservation in reservations]
        low_price = sorted(reservation_list, key=lambda k: k['basic_price'])
        return JsonResponse({"message":"SUCCESS!", "flight_list":low_price}, status=200)

class AirplaneTaxeView(View):  
    def get(self, request):
        basic_price  = Reservation.objects.get(basic_price = request.reservation.basic_price)
        fuel_surcharge = basic_price * 0.1
        customs_duty   = 8000
        ticketing_fee  = 0
